...
; write response and RT data
  (with-open-file (out (ensure-directories-exist (merge-pathnames (concatenate 'string directory *model* "VPN" (write-to-string vpn) "Rep" (write-to-string repetitions) ".csv") *current-directory*)) :direction :output :if-does-not-exist :create :if-exists :append)
...
; write BOLD predicitons to file
  (with-open-file (out (ensure-directories-exist (merge-pathnames (concatenate 'string directory *model* "VPN" (write-to-string vpn) "Rep" (write-to-string repetitions) "_BOLD.csv") *current-directory*)) :direction :output :if-does-not-exist :create :if-exists :append)
...
;   :buffer-tracing-step 0.025; trace buffer every 25ms
   ...
   :trace-detail low;high ;amount of detail shown in process trace
...
;; Define chunks and place them in declarative memory
  (add-dm
   ...
   (pair1 first-cue cue1 second-cue cue2)
   (pair2 first-cue cue2 second-cue cue3)
   (pair3 first-cue cue3 second-cue cue4)
   )
...
+visual>
	...
	screen-pos =visual-location
=goal>
	state item2-retrieve   
)


(p retrieve-cue-item2
=goal>
    state item2-retrieve
=visual>
    value       =item
